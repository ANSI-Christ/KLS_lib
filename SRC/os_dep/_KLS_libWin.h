...
#define _NET_ERRNO_TRANSLATE(_1_) M_FOREACH_A(__NET_ERR2POSIX,_1_,NOTSOCK,INTR,TIMEDOUT,OPNOTSUPP,NETUNREACH,DESTADDRREQ,MSGSIZE,PROTOTYPE,ADDRINUSE,ADDRNOTAVAIL,NETDOWN,NETRESET,CONNABORTED,CONNRESET,NOBUFS,ISCONN,NOTCONN,CONNREFUSED,HOSTUNREACH,ALREADY,INPROGRESS,AGAIN,WOULDBLOCK)
#define __NET_ERR2POSIX(_1_,_2_) if(_1_==M_JOIN(WSAE,_2_)) return M_JOIN(E,_2_);

#define _NET_ERRNO     (errno=_NET_errnoTranslate(WSAGetLastError()))
