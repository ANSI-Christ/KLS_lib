...
static KLS_byte __KLS_testAtributeVal=0;
KLS_ATTR(constructor) void _KLS_attrTest(){__KLS_testAtributeVal=1;}
KLS_byte _KLS_libTestSort(const char *prev,const char *next,void *arg){ return strlen(prev)>strlen(next); }

void KLS_libTest(){
    const int t=1;
    const char *res[]={"["KLS_TRM(EPIC,BOLD,fRED)("bad")"]", "["KLS_TRM(BOLD,fYELLOW)("ok")"]"};
    char buffer[30][256]={{0}};
    int cnt=0;
    KLS_libInit();

    {// MACROS OVERLOAD
        int bla=0;
        #define _KLS_TEST(...) M_OVERLOAD(_KLS_TEST_,__VA_ARGS__)(__VA_ARGS__)
        #define _KLS_TEST_0() bla|=1
        #define _KLS_TEST_1(a) bla|=2;
        #define _KLS_TEST_2(a,b) bla|=4;
        _KLS_TEST(); _KLS_TEST(1); _KLS_TEST(1,2);
        #undef _KLS_TEST
        #undef _KLS_TEST_0
        #undef _KLS_TEST_1
        #undef _KLS_TEST_2
        snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test macros overload %s",(bla==7)[res]);
    }

    // PAUSE & RUNTIME
    snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test pause/runtime %s",(!KLS_CMP( KLS_RUNTIME(
        KLS_pausef(0.25);
        KLS_RUNTIME(KLS_pause(0,250,0,0));
    ),0.5,0.1))[res]);

    // FUNCTIONS ATTRIBUTES
    snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test attribute %s",__KLS_testAtributeVal[res]);

    // NAMELESS ARRAY
    snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test nameless array %s",(*(int*)KLS_$(int)(10)==10)[res] );

    {// LAMBDA
        int (*f)()=KLS_LMBD(int,(){return t;});
        snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test lambda %s", f()[res]);
    }

    // IS CONST EXPR
    snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test const expressions %s", ((KLS_IS_CONST(1)==1) && (KLS_IS_CONST(t)==0))[res] );

    // MACROS ARGS COUNT
    snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test args count%s",(M_COUNT(1,2,3,4)==4 && M_COUNT()==0)[res]);

    // ABSTRACT
    snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test abstract %s", (sizeof(KLS_ABSTRACT(KLS_t_POINT)->x)==sizeof(double))[res] );

    // VALUE_SIGN
    snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test value sign %s", ((KLS_SIGN(6)==1) && (KLS_SIGN(-6)==-1))[res] );

    {//NONE CODE
        int n0=KLS_NONCODE(int), m0=-10;
        float n1=KLS_NONCODE(float), m1=-10.;
        double n2=KLS_NONCODE(double), m2=-10.;
        snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test none code %s",
                    ((KLS_IS_NONCODE(n0)==1) && (KLS_IS_NONCODE(n1)==1) && (KLS_IS_NONCODE(n2)==1) &&
                       (KLS_IS_NONCODE(m0)==0) && (KLS_IS_NONCODE(m1)==0) && (KLS_IS_NONCODE(m2)==0))[res]);
    }

    {// INF/NAN
        int i, tp[3]={1,1,1};
        const KLS_byte r[4]={1,0,0,0};
        const float a[4]={-25.0, 1.0/0.0, -1.0/0.0, acos(2.0)};
        const double b[4]={-25.0, 1.0/0.0, -1.0/0.0, acos(2.0)};
        const long double c[4]={-25.0, 1.0/0.0, -1.0/0.0, acos(2.0)};
        for(i=0;i<KLS_ARRAY_LEN(r);++i)
            tp[0]*=(KLS_IS_CORRECT(a[i])==r[i]);
        for(i=0;i<KLS_ARRAY_LEN(r);++i)
            tp[1]*=(KLS_IS_CORRECT(b[i])==r[i]);
        for(i=0;i<KLS_ARRAY_LEN(r);++i)
            tp[2]*=(KLS_IS_CORRECT(c[i])==r[i]);
        snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test inf/nan %s %s %s",tp[0][res],tp[1][res],tp[2][res]);
    }

    {// __fastcall
        KLS_byte f1(int a,double b, int c, double d){
            return a==1 && b==3.5 && c==2 && d==4.5;
        }
        KLS_byte (*f2)(int a, int c, double b, double d) = (void*)f1;
        snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test fastcall %s",(f1( 1, 3.5,   2, 4.5 )==f2( 1,   2, 3.5, 4.5 ))[res]);
    }

    {// VECTOR
        int i, cmp=1;
        KLS_t_VECTOR v=KLS_vectorNew(11,sizeof(int),NULL),c;
        for(i=0;i<11;++i)
            KLS_vectorPushBack(&v,&i);
        cmp*= (v.size==11);
        c=KLS_vectorCopy(&v);
        cmp*= (c.data && c.size==11 && c.size==c.sizeReal);
        cmp*= (KLS_vectorIndexOf(&c,&t,NULL)==1);
        KLS_vectorRemove(&c,8);
        cmp*= (c.size==10 && c.sizeReal==11 && (*(int*)KLS_vectorAt(&c,8))==9);
        i=99;KLS_vectorInsert(&v,2,&i);
        cmp*= ( (*(int*)KLS_vectorAt(&v,3)==2) && (*(int*)KLS_vectorAt(&v,2)==99));
        KLS_vectorFree(&v);
        KLS_vectorFree(&c);
        cmp*= !c.data && !v.data && !c.size && !v.size;
        snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test vector %s",cmp[res]);
    }

    {// LIST
        int i, cmp=1;
        KLS_t_LIST l=KLS_listNew(sizeof(int),NULL),c;
        for(i=0;i<10;++i)
            KLS_listPushBack(&l,&i);
        KLS_listRemoveAt(&l,2);
        i=99; KLS_listPushFront(&l,&i);
        i=88; KLS_listPushFront(&l,&i);
        i=44; KLS_listInsert(&l,&i,4);
        c=KLS_listCopy(&l);
        cmp*= (*(int*)KLS_listAt(&c,3))==1 && (*(int*)KLS_listAt(&c,1))==99 && (*(int*)KLS_listAt(&c,5))==3;
        KLS_listClear(&l);
        KLS_listClear(&c);
        cmp*= !l.first && !l.last && !l.size && !c.first && !c.last && !c.size;
        snprintf(buffer[cnt++],KLS_ARRAY_LEN(*buffer)-1,"KLS: test list %s",cmp[res]);
    }
    ...
    // sort and output (DONT TOUCH)
    KLS_arraySort(buffer,cnt,sizeof(*buffer),(void*)_KLS_libTestSort,NULL);
    while(cnt--)
        printf("%s\n",buffer[cnt]);
}
